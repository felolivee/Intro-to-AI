...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for posDist in livingGhostPositionDistributions:
            maxPos = max(posDist, key=posDist.get)
            dic[maxPos] = self.distancer.getDistance(maxPos, pacmanPosition)

        closestGhostPos = min(dic, key=dic.get)
        shortestDist = dic[closestGhostPos]

        for action in legal:
            successorPos = Actions.getSuccessor(pacmanPosition, action)
            distFromSuccToGhost = self.distancer.getDistance(successorPos, closestGhostPos)
            if distFromSuccToGhost < shortestDist:
                bestAction = action
                shortestDist = distFromSuccToGhost
        return bestAction