...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in ghostPositions:
            updatedBeliefs[ghostPos] = self.getObservationProb(observation, pacPos, ghostPos, jailPos) * self.beliefs[ghostPos]
    ...
    def elapseTime(self, gameState):
        ...
        for pos in ghostPositions:
            ...
            for key in newPosDist.keys():
                d[key] += self.beliefs[pos] * newPosDist[key]
        self.beliefs = d
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(int(numOfParticles / len(legalPos))):
            for pos in legalPos:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        posParticles = self.particles
        numParticles = self.numParticles
        jailPos = self.getJailPosition()

        for pos in posParticles:
            distribution[pos] += self.getObservationProb(observation, pacmanPos, pos, jailPos)

        for val in distribution.values():
            if val != 0:
                zeroDist = False
        self.initializeUniformly(gameState)
        #if the distribution is filled with non-zero values
        if not zeroDist:
            self.particles = []
    ...
    def elapseTime(self, gameState):
        ...
        for index, particle in enumerate(particlePos):
            samples.append(self.getPositionDistribution(gameState, particle).sample())
