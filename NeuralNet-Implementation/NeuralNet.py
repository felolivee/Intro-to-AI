...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """
        inActs.insert(0, 1.0)
        biasedWeightedSum = self.getWeightedSum(inActs)
        inActs.pop(0)
        output = self.sigmoid(biasedWeightedSum)
        return output
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        inActs.insert(0, 1.0)
        value = self.getWeightedSum(inActs)
        inActs.pop(0)
        return self.sigmoidDeriv(value)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
            newWeight = weight + (alpha * inActs[ind] * delta)
            self.weights[ind] = newWeight
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            layerActs = []
            for node in layer:
                layerActs.append(node.sigmoidActivation(lastLayer))
            neuralNet.append(layerActs)
            lastLayer = layerActs
        return neuralNet
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    for nextNeuron in range(len(nextLayer)):
                        weightsIJ = nextLayer[nextNeuron].weights[neuronNum + 1]
                        deltaJ = deltas[0][nextNeuron]
                        sum += weightsIJ * deltaJ
                    delta = gPrime * sum
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    """
    1. Get the accuracy of your Neural Network on the test examples.
    2. For each text example, you should first feedforward to get the NN outputs. Then, round the list of outputs from the output layer of the neural net.
    3. If the entire rounded list from the NN matches with the known list from the test example, then add to testCorrect, else add to  testError.
    """
    ...
    for input, output in examplesTest:
        layers = nnet.feedForward(input)
        lastLayer = layers[-1]
        isEqual = True
        for i in range(len(lastLayer)):
            roundedOut = round(lastLayer[i])
            exampleOut = output[i]
            if roundedOut != exampleOut:
                isEqual = False
        if isEqual == False:
            testError += 1
        else:
